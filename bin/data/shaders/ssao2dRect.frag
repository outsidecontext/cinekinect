...
uniform sampler2DRect texture1; // rendered scene
...
float readDepth( in vec2 coord ) {
	return (readDepthVal * camerarange.x) / (camerarange.y + camerarange.x - texture2DRect( texture0, coord ).x * (camerarange.y - camerarange.x));
    //return texture2DRect( texture0, coord ).x;
}
...
{	
	...
	gl_FragColor = vec4(1.0-ao) * texture2DRect(texture1, texCoord);
